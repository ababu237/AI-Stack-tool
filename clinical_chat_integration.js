...
class ClinicalChatAPI {
    constructor() {
        this.baseURL = 'http://localhost:8001'; // Clinical Chat API port
    ...
    async sendMessage() {
        ...
        try {
            ...
            // session_id not used by backend currently; omitted deliberately

            const response = await fetch(`${this.baseURL}/clinical_chat`, {
                method: 'POST',
                body: formData
            });
            ...
            const responseText = result.translated_response || result.response || result.answer || 'I received your message.';
            this.addMessageToChat('assistant', responseText);

            // If audio_url present, append an audio message unobtrusively
            if (result.audio_url) {
                this.addInlineAudio(result.audio_url);
            }
